...
export default {
  ...
  computed: {
    ...
    initData() {
      let { childDomains, currentResourceId } = this;
      if (!childDomains || childDomains.length == 0 || !currentResourceId) {
        return;
      }
      return { childDomains, currentResourceId };
    }
  },
  watch: {
    initData(val) {
      ...
      let { childDomains, currentResourceId } = val,
        ids = [currentResourceId].concat(childDomains);
