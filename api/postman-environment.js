...
let gen = ["192.168.1.191", "10.26.10.91", "10.27.16.133:8090", "101.132.188.149:20009"].entries();

function camelhill(str) {
  let arr = str.split("-").map((d, i) => {
    if (d == "ui") {
      return "UI"
    }
    if (d == "kpidata") {
  ...
  node.children(n => {
    ...
    .then(children => {
      let values = children.map(child => {
        let name = camelhill(child.basename);
        return {
          enabled: true,
          key: name,
...
function callSeries({ value, done }) {
  if (done == false) {
    render(value[1], () => {
      callSeries(gen.next())
    });
