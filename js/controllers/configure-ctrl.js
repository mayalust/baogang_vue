define(['controllers/controllers', 'bootstrap-dialog'], function (controllers, BootstrapDialog) {
  ...
  controllers.controller('ConfigureCtrl', ['$scope', '$q', '$rootScope', '$location', '$timeout', 'growl', 'userLoginUIService', 'resourceUIService', 'viewFlexService', 'userDomainService', 'solutionUIService', 'unitService', 'configUIService',
    function ($scope, q, $rootScope, $location, $timeout, growl, userLoginUIService, resourceUIService, viewFlexService, userDomainService, solutionUIService, unitService, configUIService) {
        ...
        function menuhandle(menuInfo) {
          ...
          if (menuInfo.function) {
            menuInfo.function.forEach(function (handlemenu) {
              ...
              if (handlemenu.url)
                $scope.menuitems[handlemenu.url] = handlemenu;
      ...
      function initViews() {
        ...
        viewFlexService.getAllMyViews(function (returnObj) {
          if (returnObj.code == 0) {
            ...
            dashboardView.push({ viewTitle: "添加自定义工作台", viewId: 0, icon: "fa fa-plus-square" });
      ...
      function callOneByOne(arr) {
        ...
        return arr.length > 1 ? getData(model).then(function (d) {
